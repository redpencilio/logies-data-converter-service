...
import { ADMS, DCT, LOCN, LOGIES, MU, PROV, RDF, SCHEMA, TVL, XSD } from './prefixes';
...
export default function mapLodgings(records, translations, errorLogger) {
  ...
  records.forEach((record) => {
    ...
    try {
      ...
      if (record.deleted) deleted = modified;
      publicG.add(sym(lodgingUri), DCT('modified'), litDateTime(modified));
    } catch (_) {
      // Invalid or no changed_time
      if (record.deleted) deleted = new Date();
    }

    if (deleted) {
      publicG.add(sym(lodgingUri), PROV('invalidatedAtTime'), litDateTime(deleted));
    } else {
