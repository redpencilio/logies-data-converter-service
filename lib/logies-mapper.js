  ...
  const statements = records
    ...
    .map(function(record) {
      ...
      if (record['number_of_units'])
        insert += `${sparqlEscapeUri(lodgingUri)} logies:aantalVerhuureenheden ${sparqlEscapeInt(record['tva_capacity'])} . `;
...
const mapLocation = function(recordId, record, lodgingUri) {
  ...
  // TODO add geometry properties
  return `
        ${sparqlEscapeUri(lodgingUri)} logies:locatie ${sparqlEscapeUri(pointUri)} .
