...
const mapRegistration = function(recordId, record, lodgingUri) {
  ...
  const insertRegistration = function(lodgingUri, decree, type, status) {
    ...
    const registrationTypeUri = registrationTypeMap[decree][type];
  ...
  if (isLogiesDecreet) {
    ...
    insert += insertRegistration(lodgingUri, decree, type, status);
  }
...
const mapLocation = function(recordId, record, lodgingUri) {
  const pointUuid = generateGeometryUuid(recordId);
  const pointUri = generateGeometryUri(pointUuid);
