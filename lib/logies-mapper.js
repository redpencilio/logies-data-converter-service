...
const mapRegistration = function(recordId, record, lodgingUri) {
  ...
  if (isLogiesDecreet) {
    const registrationUuid = generateRegistrationUuid(recordId, 'logiesDecreet');
    const registrationUri = generateRegistrationUri(registrationUuid);

    const registrationIdentifierUuid = generateRegistrationIdentifierUuid(recordId, 'logiesDecreet');
  ...
  if (isTVA) {
    const registrationUuid = generateRegistrationUuid(recordId, 'tvaDecreet');
    const registrationUri = generateRegistrationUri(registrationUuid);

    const registrationIdentifierUuid = generateRegistrationIdentifierUuid(recordId, 'tvaDecreet');
    const registrationIdentifierUri = generateIdentifierUri(registrationIdentifierUuid);
    ...
    const registrationStatusUri = registrationStatusMap[record['tva_acknowledgement']];
    if (registrationStatusUri)
      insert += ` ${sparqlEscapeUri(registrationUri)} logies:registratieStatus ${sparqlEscapeUri(registrationStatusUri)} . `;
  ...
  // TODO insert niet-tijdsgebonden registration and take registration dates into account
