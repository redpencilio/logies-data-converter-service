...
const mapBaseRegistry = function(records) {
  const statements = records.map(function(record) {
    ...
    const lodgingUuid = uriGenerator.lodgingUuid(recordId);
    const lodgingUri = uriGenerator.lodgingUri(lodgingUuid);

    const lodgingIdentifierUuid = uriGenerator.lodgingIdentifierUuid(recordId);
    const lodgingIdentifierUri = uriGenerator.identifierUri(lodgingIdentifierUuid);
...
const mapRegistration = function(recordId, record, lodgingUri) {
  ...
  const insertRegistration = function(lodgingUri, decree, type, category, status, issuedDate) {
    const registrationUuid = uriGenerator.registrationUuid(recordId, decree);
    const registrationUri = uriGenerator.registrationUri(registrationUuid);

    const registrationIdentifierUuid = uriGenerator.registrationIdentifierUuid(recordId, decree);
    const registrationIdentifierUri = uriGenerator.identifierUri(registrationIdentifierUuid);
...
const mapAlternateExploitations = function(recordId, record, lodgingUri) {
  ...
  if (record['product_type'] == 'PROMO') {
    ...
    if (parents && parents.length) {
      const parentLodgingUris = parents.map(id => uriGenerator.lodgingUri(uriGenerator.lodgingUuid(id)));
...
const mapLocation = function(recordId, record, lodgingUri) {
  const pointUuid = uriGenerator.geometryUuid(recordId);
  const pointUri = uriGenerator.geometryUri(pointUuid);
...
const mapAddress = function(recordId, record, lodgingUri) {
  const addressUuid = uriGenerator.addressUuid(recordId);
  const addressUri = uriGenerator.addressUri(addressUuid);
...
const mapCapacities = function(recordId, record, lodgingUri) {
  ...
  columns.forEach(function(key) {
    if (record[key]) {
      ...
      const quantitativeValueUuid = uriGenerator.quantitativeValueUuid(recordId, unit);
      const quantitativeValueUri = uriGenerator.quantitativeValueUri(quantitativeValueUuid);
...
const mapRating = function(recordId, record, lodgingUri, field = 'comfort_class') {
  if (record[field]) {
    const ratingUuid = uriGenerator.ratingUuid(recordId, field);
    const ratingUri = uriGenerator.ratingUri(ratingUuid);
...
const mapGreenlabel = function(recordId, record, lodgingUri) {
  const labelUuid = uriGenerator.qualityLabelUuid(recordId, 'green-label');
  const labelUri = uriGenerator.qualityLabelUri(labelUuid);
...
const mapAccessibilityLabel = function(recordId, record, lodgingUri) {
  if (record['accessibility_label']) {
    const labelUuid = uriGenerator.qualityLabelUuid(recordId, 'accessibility');
    const labelUri = uriGenerator.qualityLabelUri(labelUuid);
...
const mapContactPoints = function(recordId, record, lodgingUri) {
  ...
  columns.forEach(function(key) {
    ...
    if (value) {
      const contactPointUuid = uriGenerator.contactPointUuid(recordId, key);
      const contactPointUri = uriGenerator.contactPointUri(contactPointUuid);
...
const mapDescription = function(recordId, record, lodgingUri) {
  if (record['product_description_nl'] || record['product_description_en']) {
    const descriptionUuid = uriGenerator.descriptionUuid(recordId);
    const descriptionUri = uriGenerator.descriptionUri(descriptionUuid);
...
const mapMediaObjects = function(recordId, record, lodgingUri) {
  ...
  return mediaObjects.map(function(contentUrl) {
    ...
    if (value) {
      const validUrlRegex = /^[\da-zA-Z-\\\.:\/?=_%&#+@]*$/g; // /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/g;
      if (validUrlRegex.test(value)) {
        const mediaObjectUuid = uriGenerator.mediaObjectUuid(recordId, contentUrl);
        const mediaObjectUri = uriGenerator.mediaObjectUri(mediaObjectUuid);
