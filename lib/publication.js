...
const publishDataDump = async function(fileUuid) {
  const { uuid: uploadResourceUuid, uri: uploadResourceUri } = uriGenerator.file(uuid());
  const { dataDumpUuid, dataDumpUri } = uriGenerator.dataDumpUri(uuid());
  const now = new Date();
