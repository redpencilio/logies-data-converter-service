...
const generationBaseUri = 'http://linked.toerismevlaanderen.be/id/generations/';
const invalidationBaseUri = 'http://linked.toerismevlaanderen.be/id/invalidations/';
...
const generateRegistrationUuid = function(id, decree, status = '') {
  return sha256(`dd7b9b41-ed08-467b-bebf-b4b7aab22291-${id}-${decree}-${status}`);
};
...
const generateGenerationUuid = function(registrationUuid) {
  return sha256(`78c89386-1600-49a7-950f-26a14984ecc7-${registrationUuid}`);
};

const generateGenerationUri = function(uuid) {
  return `${generationBaseUri}${uuid}`;
};

const generateInvalidationUuid = function(registrationUuid) {
  return sha256(`94405402-2580-4483-826e-26b41591dc09-${registrationUuid}`);
};

const generateInvalidationUri = function(uuid) {
  return `${invalidationBaseUri}${uuid}`;
};
